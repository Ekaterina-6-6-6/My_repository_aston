...
public class MtsPage {
    ...
    private final By paymentTypeSelector = By.cssSelector(".pay .select__header");
    ...
    private final By connectionAmount = By.id("connection-sum");
    private final By connectionEmail = By.id("connection-email");

    private final By connectionContinueButton =
            By.xpath("//form[@id='pay-connection']//button[normalize-space()='Продолжить']");

    private final By internetPhone = By.id("internet-phone");
    private final By internetAmount = By.id("internet-sum");
    private final By internetEmail = By.id("internet-email");

    private final By instalmentAccount = By.id("score-instalment");
    private final By instalmentAmount = By.id("instalment-sum");
    private final By instalmentEmail = By.id("instalment-email");

    private final By arrearsAccount = By.id("score-arrears");
    private final By arrearsAmount = By.id("arrears-sum");
    private final By arrearsEmail = By.id("arrears-email");

    private final By paymentFrame =
            By.cssSelector( "iframe[src*='checkout.bepaid.by/widget_v2']" );
    ...
    private final By paymentVisaLogo =
            By.cssSelector(".cards-brands__container img[src*='visa-system']");

    private final By paymentMasterCardLogo =
            By.cssSelector(".cards-brands__container img[src*='mastercard-system']");
    ...
    public String getPaymentBlockTitle() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentBlockTitle)
        ).getText().replaceAll("\\s+", " ").trim();
    }

    public void selectPaymentType(String paymentType) {
        wait.until(
                ExpectedConditions.elementToBeClickable(paymentTypeSelector)
        ).click();

        By paymentTypeOption = By.xpath(
                "//div[contains(@class,'pay')]//li[contains(@class,'select__item')]" +
                        "[.//p[contains(@class,'select__option') and normalize-space()='" +
                        paymentType + "']]"
        );

        wait.until(
                ExpectedConditions.elementToBeClickable(paymentTypeOption)
        ).click();
    }

    public boolean isVisaLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(visaLogo)
        ).isDisplayed();
    }

    public boolean isMasterCardLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(masterCardLogo)
        ).isDisplayed();
    }

    public boolean isBelkartLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(belkartLogo)
        ).isDisplayed();
    }

    public String getServiceDetailsLinkUrl() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(serviceDetailsLink)
        ).getAttribute("href");
    }

    public void clickServiceDetailsLink() {
        wait.until(
                ExpectedConditions.elementToBeClickable(serviceDetailsLink)
        ).click();
    }
    ...
    public String getConnectionPhone() {
        return getFieldValue(connectionPhone).replaceAll("\\D", "");
    }
    ...
    public boolean isConnectionContinueButtonEnabled() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(connectionContinueButton)
        ).isEnabled();
    }
    ...
    public void clickConnectionContinueButton() {
        ...
        wait.until( ExpectedConditions.presenceOfElementLocated(paymentFrame) );
    }
    ...
    public void switchToPaymentFrame() {
        ...
        wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentAmount) );
    }
    ...
    public String getPaymentPhone() {
        String description = wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentDescription)
        ).getText();

        return description.substring(
                description.indexOf("Номер:") + "Номер:".length()
        ).trim();
    }
    ...
    public String getPaymentButtonText() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentButton)
        ).getText().replaceAll("\\s+", " ").trim();
    }

    public boolean isPaymentVisaLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentVisaLogo)
        ).isDisplayed();
    }

    public boolean isPaymentMasterCardLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentMasterCardLogo)
        ).isDisplayed();
    }

    public boolean isPaymentBelkartLogoDisplayed() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(paymentBelkartLogo)
        ).isDisplayed();
    }
    ...
    private String getFieldLabel(By fieldLocator) {
        ...
        return field.findElement(
                By.xpath("./ancestor::*[.//label][1]//label")
        ).getText().trim();
    }
}
